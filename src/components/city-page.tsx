...
import { loadCitiesContent, type CityContent, DEFAULT_CITY_CONTENT } from '@/lib/citiesAdmin';
...
const IMAGE_FALLBACKS: Record<string, string> = {
  london: '/images/london-big.jpg',
  amsterdam: '/images/london-big.jpg',
  singapore: '/images/london-big.jpg',
  barcelona: '/images/london-big.jpg',
...
export default function CityPage({ slug }: CityPageProps) {
  const [cityContent, setCityContent] = useState<CityContent>(DEFAULT_CITY_CONTENT[slug] || DEFAULT_CITY_CONTENT.london);
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        const content = await loadCitiesContent();
        if (!cancelled) {
          setCityContent(content[slug] || DEFAULT_CITY_CONTENT[slug] || DEFAULT_CITY_CONTENT.london);
        }
      } catch (error) {
        // Failed to load city content
      } finally {
        if (!cancelled) {
          setIsLoading(false);
        }
    ...
    <main className="min-h-screen w-full bg-white">
        ...
        <div className="relative z-10 flex flex-col min-h-[700px]">
          ...
          <div className="flex-1 w-full max-w-[1280px] mx-auto px-2 flex flex-col items-center justify-start pt-16">
            <div className="text-white text-center w-full">
              <h1 className="text-6xl sm:text-8xl font-extrabold tracking-tight">{cityContent.title}</h1>
              <p className="mt-2 text-2xl sm:text-3xl font-semibold">{cityContent.subtitle}</p>
      ...
      <section className="max-w-[1280px] mx-auto px-2 pb-4 sm:pb-10 text-center">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div className="rounded-2xl bg-[#0ABAB5] p-6 text-white text-center">
            ...
            <p className="mt-2">
              Find competitive prices on business & first class flights to {cityContent.title} that aren't available elsewhere.
            </p>
          </div>
          <div className="rounded-2xl bg-[#0ABAB5] p-6 text-white text-center">
      ...
      <section className="max-w-[1280px] mx-auto px-2 py-10 sm:py-14 text-center">
        ...
        <p className="text-gray-900 text-lg sm:text-xl font-semibold max-w-4xl mx-auto">
          {cityContent.ctaTitle || `Book Your ${cityContent.title} Business Class Flight Today.`}
        </p>
        <p className="mt-3 text-gray-700 text-lg leading-relaxed max-w-4xl mx-auto">
          {cityContent.ctaText || `Let Fly Lux Sky elevate your journey to ${cityContent.title} with unmatched luxury and convenience. Contact our expert travel advisors to secure your exclusive fare and experience like never before.`}
        </p>
      </section>
    </main>
  );
